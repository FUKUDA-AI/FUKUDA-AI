...
- AgentはStatus: released / verified のみを参照対象とする（KNOWLEDGE_PROMOTION_RULES準拠。矛盾時はverified優先）
- verified（会社標準Knowledge）への昇格はCEOのみ。AIは候補提案まで
...
version: v1.0
status: released        # draft / in_review / released / verified / rejected（Lifecycle: 02_Rules/KNOWLEDGE_PROMOTION_RULES.md §1）
reviewer: CEO
---
...
## 現在の状態（2026-07-06）
- **Released Knowledge 12件**（01_Brands 1 / 02_Products 3 / 03_Customers 2 / 05_Events 3 / 10_Sales 1 / 04_SOP 2）— **Agentの参照対象**
- in_review 1件（KN-SOP-0003・催事設営チェックリスト。実運用で成熟後にReleased昇格）
- Knowledge Builder v1.1 [Experimental]。`python3 knowledge_builder.py` で転記+索引再生成（冪等）

## 今後のTODO
- KN-SOP-0003の実運用・成熟判断
