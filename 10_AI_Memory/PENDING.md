...
| 10 | ~~**so u発注量の判断材料（Brief#3判断3・Hold）**~~ ✅ **2026-07-18 クローズ（CEO: タスク自体がテストだったため不要）**。追跡終了。fos_importer v1.2.3で決着済みstaffRequest（approved/rejected）を要判断キューから除外済み（テスト相談がBriefに載り続けない） | — | 完了 |
| 11 | ~~工場打ち合わせResult初号~~ → **RES-0001として記録済み（2026-07-07・継続観察）**。残: 打ち合わせメモ受領でEvidence補完+納期/品質/生産結果が出た時点で最終判定（成功/失敗） | CEO任意 | 低 |
| 12 | LINE相談ボット（スタッフ相談のLINE完結化）: **先送り**（CEO判断 2026-07-11・窓口増による管理負担を考慮）。成果物は06_Reports/staff_board/に保管済み（雛形xlsx+設置手順v2・着手時40分）。当面の相談はFOS tasksに【相談】プレフィックスで運用 | CEO任意 | 低 |
...
- **EC/物流データのAPI接続（2026-07-18 保留・CEO判断）**: ④会社の状態を実数値化するため、はぴロジ（全注文）またはShopify/MakeShopの接続を検討。①CSVエクスポートは不可 ②はぴロジの公開API連携は「カート→はぴロジ」向けで、外部が"読み出す"公開APIは未確認 → 再開時は【A】はぴロジ管理画面/サポートで読み出しAPI（取得/Webフック）の有無を確認 or 【B】Shopify Admin API（読み出しが整備・確実）から接続。APIキーはチャットに貼らず、Mac側のgit除外ファイル/環境変数に置きimporterが読む方式。台帳: DS-LOG-0001（はぴロジ・record_schema=ShipmentRecord・pii=high→PII除外）。**現状の④はAirレジ売上＋注意点自動検出で運用可**
