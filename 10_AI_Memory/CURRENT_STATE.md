...
- **Brief v2.1 実装Sprint 1完了（2026-07-18）**: **FOS Rule v1.3 + fos_importer v1.3.0** — ai_ready属性（yes/no/null）新設・importer透過（推測しない）・ai_action_candidate（yes×未完了）付与・index.jsonにai_actions件数/records出力。テスト全合格（実データ0件・合成で透過/ガード確認・FOS不変）。**次: 実装Sprint 2 = ceo_assistant v2.1**（おはよう起動・AIから一言・判断1件絞り込み・FOS Review 6観点+3件厳選・AI Actions表示・30行ガード）
