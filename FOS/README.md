# FOS Operating Rule v1.3 — FOS運用ルール + Decision Metadata + ai_ready

Version: v1.3（Brief v2.1 実装Sprint 1・**ai_ready属性を追加** = AI Actions＝AI秘書化の前提。v1.2＝Decision Metadata）
最終更新日: 2026-07-18
状態: **Released**（CEO承認: v1.2=2026-07-07 Sprint 14系 / ai_ready追加=Brief v2.1承認に伴い2026-07-18）
...
### 4-6. ai_ready（AIが今すぐ終わらせられる仕事か・v1.3追加）

Decision Metadata（§4-1〜4-5）が「CEOの判断」を分類するのに対し、**ai_ready は「AIに任せられる作業か」**を示す別軸の属性である（Morning Brief v2.1 §③ AI Actions の入口）。

| 項目 | 内容 |
|---|---|
| 属性名 | `ai_ready`: **yes / no**（未入力=**null**=未判定・推測で埋めない） |
| 入力者 | **CEO・スタッフ**（FOS入力時）。AIは ai_ready=yes の候補を**提案できるが確定しない**（FOS Reviewの提案として提示） |
| **yesの目安** | メール返信作成 / CSV取込 / データ同期 / Dashboard更新 / 契約書ドラフト / 売上レポート作成 など、**Draft・取込・生成・整理で完結する仕事** |
| **noの目安** | 対外送信・支払・発注・交渉・現場作業など、**人の実行・判断を含む仕事** |
| Briefへの流れ | fos_importer が ai_ready を TaskRecord へ透過（推測で埋めない）→ `ai_action_candidate`（ai_ready=yes かつ未完了）→ **Brief §③ AI Actions へ自動掲載** → CEO承認 → AIが実行（**Draft/取込/生成まで**）→ 完了報告 → **FOS完了化はCEO操作** |

- **不変の制約（AI_CHARTER準拠）**: AIが「実行」できるのは Draft作成・取込・生成・整理まで。**対外送信・支払・発注・FOS変更・Knowledge released化はCEOのみ**（「メール作成」=下書きまで・送信はCEO）。
- 迷ったら空欄（null）でよい。AIがFOS Reviewで「これはAIでできます（ai_ready=yes候補）」と提案する。**確定入力はCEO/スタッフ**。

## 5. FOS入力テンプレート（v1.3・15項目）
...
ai_ready:            yes / no（AIだけで完結する仕事=Draft/取込/生成/整理ならyes・対外送信/支払/発注/現場はno・不明は空欄=AIが提案。§4-6）
```
...
| 2026-07-18 | v1.3 | **ai_ready属性を追加**（§4-6）。yes/no/null・入力者はCEO/スタッフ・AIは提案のみ確定しない。テンプレート15項目化。Morning Brief v2.1 §③ AI Actions の入口（Brief v2.1承認に伴う実装Sprint 1）。fos_importer v1.3が透過・ai_action_candidate付与 |
