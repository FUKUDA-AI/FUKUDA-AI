...
# -*- coding: utf-8 -*-
"""
Dashboard Generator v1.0 [Experimental] — NOMADO株式会社 FUKUDA AI プロジェクト（Sprint 15.2）
...
def company_health(fos_idx, learning):
    ...
    if fos_idx:
        ...
        waiting = sum(1 for r in recs if r.get("source_type") == "staff_request")
    ...
    return {"score": score, "max": max_score, "items": items,
            "excluded": "売上20・利益15・現金15・在庫10・出荷10（計70点分）= 未接続のため算定対象外"}
...
def todays_dashboard(fos_idx, today):
    rows = []
    if fos_idx:
        recs = fos_idx.get("records", [])
        waiting = [r for r in recs if r.get("source_type") == "staff_request"]
    ...
    for name in ("本日の売上 / 催事売上 / EC売上", "受注件数 / 出荷件数 / 未出荷", "入金予定"):
        rows.append((name, "未接続（Airレジ/Shopify/FLAM/はぴロジ/Airペイ接続後に表示）"))
    return rows
...
def main():
    ...
    todays = todays_dashboard(fos_idx, today)
